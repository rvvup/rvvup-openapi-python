# coding: utf-8

"""
    Rvvup API

    Rvvup Public API

    The version of the OpenAPI document: 2024-03-01
    Contact: info@rvvup.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.page_payment_method_detail import PagePaymentMethodDetail

class TestPagePaymentMethodDetail(unittest.TestCase):
    """PagePaymentMethodDetail unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> PagePaymentMethodDetail:
        """Test PagePaymentMethodDetail
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `PagePaymentMethodDetail`
        """
        model = PagePaymentMethodDetail()
        if include_optional:
            return PagePaymentMethodDetail(
                pageable = openapi_client.models.pageable.Pageable(
                    limit = 56, 
                    offset = 56, ),
                results = [
                    openapi_client.models.payment_method_detail.PaymentMethodDetail(
                        limits = openapi_client.models.payment_method_limit.PaymentMethodLimit(
                            expires_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                            total = [
                                openapi_client.models.payment_method_total_limit.PaymentMethodTotalLimit(
                                    currency = '', 
                                    max = '', 
                                    min = '', )
                                ], ), 
                        logo_url = '', 
                        name = 'FAKE_PAYMENT_METHOD', 
                        settings = openapi_client.models.payment_method_settings.PaymentMethodSettings(
                            assets = [
                                openapi_client.models.payment_method_asset.PaymentMethodAsset(
                                    asset_type = 'SCRIPT', 
                                    attributes = {
                                        'key' : ''
                                        }, 
                                    url = '', )
                                ], 
                            description = '', 
                            display_name = '', ), 
                        status = 'ENABLED', 
                        summary_url = '', )
                    ],
                total = 56
            )
        else:
            return PagePaymentMethodDetail(
                pageable = openapi_client.models.pageable.Pageable(
                    limit = 56, 
                    offset = 56, ),
                results = [
                    openapi_client.models.payment_method_detail.PaymentMethodDetail(
                        limits = openapi_client.models.payment_method_limit.PaymentMethodLimit(
                            expires_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                            total = [
                                openapi_client.models.payment_method_total_limit.PaymentMethodTotalLimit(
                                    currency = '', 
                                    max = '', 
                                    min = '', )
                                ], ), 
                        logo_url = '', 
                        name = 'FAKE_PAYMENT_METHOD', 
                        settings = openapi_client.models.payment_method_settings.PaymentMethodSettings(
                            assets = [
                                openapi_client.models.payment_method_asset.PaymentMethodAsset(
                                    asset_type = 'SCRIPT', 
                                    attributes = {
                                        'key' : ''
                                        }, 
                                    url = '', )
                                ], 
                            description = '', 
                            display_name = '', ), 
                        status = 'ENABLED', 
                        summary_url = '', )
                    ],
                total = 56,
        )
        """

    def testPagePaymentMethodDetail(self):
        """Test PagePaymentMethodDetail"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
